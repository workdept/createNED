...
class Awaken_Video_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'awaken_pro_video_widget', // Base ID
			__( 'Awaken: Youtube Video', 'awaken-pro' ), // Name
			array( 'description' => __( 'Displays a youtube video.', 'awaken-pro' ), ) // Args
		);
	}
	...
	public function form( $instance ) {
		//print_r($instance);
		$defaults = array(
			'title'		=>	__( 'Featured Video', 'awaken-pro' ),
			'vid_url'	=>	'SQEQr7c0-dw'
		);
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'vid_url' ); ?>"><?php _e( 'Youtube Video ID', 'awaken-pro' ); ?></label>
...
// register widget
function register_awaken_pro_video_widget() {
    register_widget( 'Awaken_Video_Widget' );
}
add_action( 'widgets_init', 'register_awaken_pro_video_widget' );