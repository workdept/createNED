...
class Awaken_Sidebar_Posts extends WP_Widget {

	/* Register Widget with WordPress*/
	function __construct() {
		parent::__construct(
			'sidebar_posts', // Base ID
			__( 'Awaken: Sidebar Posts', 'awaken-pro' ), // Name
			array( 'description' => __( 'Displays latest posts or posts from a choosen category.Use this widget to sidebars except magazine sidebars.', 'awaken-pro' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		<div class="awaken-block" id="<?php echo $block_uid ?>">
            ...
            <?php if( !is_rtl() ) : ?>
                ...
                <?php if ($latest_posts->max_num_pages > 1 && is_front_page()) { // check if the max number of pages is greater than 1  ?>
...
function register_awaken_pro_sidebar_posts() {
    register_widget( 'Awaken_Sidebar_Posts' );
}
add_action( 'widgets_init', 'register_awaken_pro_sidebar_posts' );