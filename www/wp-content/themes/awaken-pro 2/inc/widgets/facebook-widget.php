<?php
/**
 *	Awaken Facebook Like Box
 */

class Awaken_Facebook_Widget extends WP_Widget {

    function __construct() {
        parent::__construct(
            'facebook-like-widget', // Base ID
            __( 'Awaken: Facebook Like Box', 'awaken-pro' ), // Name
            array( 'description' => __( 'Add Facebook Like Box.', 'awaken-pro' ), ) // Args
        );
    }
	...
	function form($instance) {
		$defaults = array('title' => __('Find us on Facebook','awaken-pro'), 'page_url' => '', 'width' => '300', 'color_scheme' => 'light', 'show_faces' => 'on', 'show_stream' => false, 'show_header' => false);
		...
		<p>
			<label for="<?php echo $this->get_field_id('page_url'); ?>"><?php _e('Facebook Page URL','awaken-pro'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('width'); ?>"><?php _e('Width','awaken-pro'); ?>:</label>
			<input type="number" class="widefat" style="width: 50px;" id="<?php echo $this->get_field_id('width'); ?>" name="<?php echo $this->get_field_name('width'); ?>" value="<?php echo $instance['width']; ?>" />
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id('color_scheme'); ?>"><?php _e('Color Scheme','awaken-pro'); ?>:</label> 
			<select id="<?php echo $this->get_field_id('color_scheme'); ?>" name="<?php echo $this->get_field_name('color_scheme'); ?>" style="width:100%;">
				<option <?php if ('light' == $instance['color_scheme']) echo 'selected="selected"'; ?>><?php _e('light','awaken-pro'); ?></option>
				<option <?php if ('dark' == $instance['color_scheme']) echo 'selected="selected"'; ?>><?php _e('dark','awaken-pro'); ?></option>
			</select>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_faces'); ?>"><?php _e('Show faces','awaken-pro'); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_stream'); ?>"><?php _e('Show stream','awaken-pro'); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_header'); ?>"><?php _e('Show facebook header','awaken-pro'); ?></label>
		</p>
	<?php
	}
}
...
add_action('widgets_init', 'register_awaken_pro_facebook_widget');

function register_awaken_pro_facebook_widget() {
	register_widget( 'Awaken_Facebook_Widget' );
}
